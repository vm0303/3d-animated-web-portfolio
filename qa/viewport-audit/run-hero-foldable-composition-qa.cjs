...
const contractPath = path.join(root, 'qa', 'viewport-audit', 'hero-foldable-composition-contract.json');
const phoneContractPath = path.join(root, 'qa', 'viewport-audit', 'hero-composition-contract.json');
const tabletContractPath = path.join(root, 'qa', 'viewport-audit', 'hero-tablet-composition-contract.json');
const coveragePath = path.join(root, 'qa-results', 'testmu', 'catalog', 'TESTMU__candidate-coverage-set__us__latest.json');
...
const outDir = earlyOutputDirValue ? path.resolve(root, earlyOutputDirValue) : path.join(root, 'qa-results', 'hero-foldable-composition');
...
if (!fs.existsSync(contractPath)) {
  console.error(`Missing foldable composition contract: ${contractPath}`);
  process.exit(1);
}
if (!fs.existsSync(phoneContractPath)) {
  console.error(`Missing phone composition contract: ${phoneContractPath}`);
  process.exit(1);
}
if (!fs.existsSync(tabletContractPath)) {
  console.error(`Missing tablet composition contract: ${tabletContractPath}`);
  process.exit(1);
}
if (!fs.existsSync(coveragePath)) {
  console.error(`Missing TestMU coverage set: ${coveragePath}`);
  process.exit(1);
}

const registry = readJson(registryPath);
const contract = readJson(contractPath);
const phoneContract = readJson(phoneContractPath);
const tabletContract = readJson(tabletContractPath);
const coverage = readJson(coveragePath);

const normalizeDeviceName = (value) =>
  String(value || '').trim().toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim();

const foldableCandidates = new Map(
  (coverage.candidates || [])
    .filter((item) => String(item.deviceType || '').toLowerCase() === 'foldable')
    .map((item) => [
      normalizeDeviceName(item.deviceName),
      {
        deviceName: item.deviceName,
        formFactor: item.foldablePolicy?.formFactor || null,
        requiredDisplayStates: new Set(item.foldablePolicy?.requiredDisplayStates || []),
      },
    ])
);

if (!foldableCandidates.size) {
  console.error('No foldable devices found in the current TestMU coverage set.');
  process.exit(1);
}
...
const layoutFamilyFor = ({ displayState, orientation, width, height }) => {
  const state = String(displayState || '');

  // Folded outer / unspanned single displays inherit the frozen phone
  // composition only when their *actual CSS geometry* is phone-like.
  // Wider/taller single-display states (for example Surface Duo) must be
  // allowed to fall through into tablet/foldable geometry families.
  if (
    state === 'folded-outer-display' ||
    state === 'single-display-or-unspanned'
  ) {
    if (orientation === 'portrait' && width <= 500) return 'phone';
    if (orientation === 'landscape' && height <= 500) return 'phone';
  }

  // Unfolded book-fold portrait naturally belongs to the already-approved
  // tablet portrait family when it is wider than the phone envelope.
  if (
    state === 'unfolded-inner-display' &&
  ...
  // Existing frozen geometry remains authoritative for synthetic or future
  // states that clearly fall into an already-certified family.
  if (orientation === 'portrait' && width <= 500) return 'phone';
  if (orientation === 'landscape' && height <= 500) return 'phone';
  if (orientation === 'portrait' && width > 500) return 'tablet';
  if (orientation === 'landscape' && width >= 1000 && height >= 501) return 'tablet';

  // This is the actual missing bridge we are certifying: medium-width,
  // medium-height unfolded/spanned foldable landscape.
  return 'foldable';
};

const contractForCase = (testCase) => {
  const family =
    testCase.layoutFamily ||
    layoutFamilyFor(testCase);

  if (family === 'phone') return phoneContract;
  if (family === 'tablet') return tabletContract;
  return contract;
};
...
const eligibleCapture = (capture) => {
  ...
  const candidate = foldableCandidates.get(normalizeDeviceName(capture.deviceName));
  ...
  // Ignore ordinary orientation-only foldable probes. Book folds and Duo
  // count only when the display/posture state is one of the explicitly
  // required states from the current TestMU foldable policy.
  if (!candidate.requiredDisplayStates.has(displayState)) return false;

  const verification = String(capture.displayVerificationStatus || '');
  const verifiedDisplayState =
    /CONFIRMED|VERIFIED/i.test(verification) &&
    !/UNVERIFIED|NOT_APPLICABLE/i.test(verification);

  const manual = capture.manualCertification || {};
  const approvedManual =
    String(capture.sourceStatus || '') === 'MANUAL_CERTIFIED' &&
    manual.approvedForPrimaryDiscovery === true &&
    manual.automationDerived === false &&
    manual.appiumDerived === false;

  const approvedAutomated =
    String(capture.sourceStatus || '') === 'PASS' &&
    String(capture.evidenceSource || '') !== 'testmu-manual-real-device';

  return verifiedDisplayState && (approvedManual || approvedAutomated);
};
...
for (const capture of registry.captures || []) {
  ...
  const displayState = String(capture.displayState || capture.requestedDisplayScope || '');
  const foldState = capture.foldState || null;
  const posture = capture.posture || null;
  const layoutFamily = layoutFamilyFor({ displayState, orientation, width, height });

  // Keep posture/display states distinct even when two states happen to share
  // the same CSS viewport geometry.
  const key = `${displayState}|${orientation}|${width}x${height}`;

  if (!geometryMap.has(key)) {
    geometryMap.set(key, {
      id: `measured-${displayState}-${orientation}-${width}x${height}`,
      source: 'MEASURED',
      orientation,
      width,
      height,
...
const cases = [...geometryMap.values()].map((item) => ({
  ...
  label:
    `${[...item.devices].slice(0, 3).join(', ')}` +
    ` — ${item.displayState}` +
    ` — ${item.layoutFamily}`,
}));

const addSynthetic = (candidate) => {
  const displayState = candidate.displayState || 'synthetic-foldable';
  const layoutFamily =
  ...
  const key = `${displayState}|${candidate.orientation}|${candidate.width}x${candidate.height}`;
  const existing = cases.find(
    (item) =>
      `${item.displayState || 'synthetic-foldable'}|${item.orientation}|${item.width}x${item.height}` === key
  );

  if (existing) {
    if (candidate.label && !existing.labels.includes(candidate.label)) {
  ...
  cases.push({
    id: `synthetic-${displayState}-${candidate.orientation}-${candidate.width}x${candidate.height}`,
    source: 'SYNTHETIC',
    orientation: candidate.orientation,
    width: candidate.width,
    height: candidate.height,
    displayState,
    foldState: candidate.foldState || null,
    posture: candidate.posture || null,
    layoutFamily,
    label: candidate.label || 'synthetic foldable probe',
...
if (quickMode) {
  const sentinels = new Set(
    (contract.synthetic.sentinels || []).map(
      (item) => `${item.displayState || 'synthetic-foldable'}|${item.orientation}|${item.width}x${item.height}`
    )
  );
...
const assessCase = (testCase, dom) => {
  const effectiveContract = contractForCase(testCase);
  const issues = [];
  const hero = dom.hero;
  const isPortrait = testCase.orientation === 'portrait';
  const { width, height } = testCase;
  const bands = isPortrait ? effectiveContract.portrait.pressureBands : effectiveContract.landscape.pressureBands;
  ...
  if (dom.heroImageMetrics) {
    const imagePolicy = effectiveContract.image || {};
  ...
  if (dom.socialsAnchors.length >= 3 && visible(dom.socials)) {
    ...
    if (spread > effectiveContract.socials.centerXSpreadMax || !verticalOrder) {
    ...
    if (topGap != null && topGap > effectiveContract.socials.topReviewMax) {
      addIssue(
        issues,
        'REVIEW',
        'SOCIALS_TOO_LOW',
     ...
     */
    const heroRightMax = Number(effectiveContract.socials.heroRightReviewMax ?? 4);
    ...
    const viewportMin = Number(effectiveContract.socials.viewportRightIdealMin ?? 10);
    const viewportMax = Number(effectiveContract.socials.viewportRightIdealMax ?? 28);
  ...
  if (visible(hero) && visible(dom.contact)) {
    ...
    if (
      ...
      const contactPolicy = effectiveContract.contact || {};
  ...
  const visualPolicy = effectiveContract.visualViewport || {};
  ...
  // visually separate in the user's Hero effectiveContract.
  ...
  if (isPortrait) {
    const bubbleShouldHide = effectiveContract.portrait.bubbleHiddenBands.includes(pressureBand);
    ...
    evaluateGap(issues, 'titleToCertifications', metrics.titleToCertifications, effectiveContract.portrait.gaps.titleToCertifications);
    if (bubbleIsVisible) {
      evaluateGap(issues, 'certificationsToBubble', metrics.certificationsToBubble, effectiveContract.portrait.gaps.certificationsToBubble);
      evaluateGap(issues, 'bubbleToScroll', metrics.bubbleToScroll, effectiveContract.portrait.gaps.bubbleToScroll);
    }
    evaluateGap(issues, 'scrollToImage', metrics.scrollToImage, effectiveContract.portrait.gaps.scrollToImage);
    ...
    if (bubbleIsVisible && effectiveContract.bubble.requireSingleLine && dom.bubbleTextMetrics) {
      const m = dom.bubbleTextMetrics;
      const nowrapFits = m.scrollWidth <= m.clientWidth + effectiveContract.bubble.horizontalTolerancePx;
      if (!nowrapFits) {
    ...
    const z = effectiveContract.landscape.zones;

    const bubbleMinViewportWidth =
      Number(
        effectiveContract.landscape
    ...
    if (
      bubbleIsVisible &&
      effectiveContract.bubble.requireSingleLine &&
    ...
    if (shortSweepMode) {
      const visualContract =
        effectiveContract.landscapeShortSweep
          ?.visualContract || {};
    ...
    const alignment = effectiveContract.landscape.imageBackgroundAlignment || {};
    ...
    const internalGaps = effectiveContract.landscape.certificationInternalGaps || {};
...
const main = async () => {
  ...
  try {
    ...
    console.log(`Foldable Hero composition QA: ${filteredCases.length} cases (${orientationFilter}).`);
    ...
    const report = {
      ...
      source: {
        ...
        phoneContractPath: path.relative(root, phoneContractPath),
        tabletContractPath: path.relative(root, tabletContractPath),
      ...
      worstCases: [...results].sort((a, b) => a.score - b.score).slice(0, 30).map((item) => ({
        ...
        width: item.width,
        height: item.height,
        displayState: item.displayState || null,
        foldState: item.foldState || null,
    ...
    const csvHeader = ['status', 'score', 'source', 'displayState', 'foldState', 'posture', 'layoutFamily', 'orientation', 'width', 'height', 'pressureBand', 'label', 'issues', 'titleToCertifications', 'certificationsToBubble', 'bubbleToScroll', 'scrollToImage', 'contactBottomGap', 'contactRightGap', 'contactRotationSafe'];
    ...
    const md = [];
    md.push('# Foldable Hero Composition QA', '');
    ...
    md.push('- Folded outer / single-display states inherit the frozen phone contract when their CSS geometry is phone-like.');
    md.push('- Unfolded inner portrait states inherit the frozen tablet portrait contract when wider than the phone envelope.');
    md.push('- Medium-width unfolded/spanned landscape states use the foldable bridge contract.');
    md.push('- Closed clamshell widget/cover screens are intentionally outside Hero scope.', '');
    ...
    md.push('## Worst cases', '');
    md.push('| Score | Status | State | Family | Geometry | Band | Main issues | Screenshot |');
    md.push('| ---: | --- | --- | --- | --- | --- | --- | --- |');
    for (const item of report.worstCases) {
      const issueText = item.issues.slice(0, 4).map((issue) => `${issue.severity}:${issue.code}`).join('<br>') || '—';
      md.push(`| ${item.score} | ${item.status} | ${item.displayState || '—'} | ${item.layoutFamily || '—'} | ${item.width}×${item.height} ${item.orientation} | ${item.pressureBand} | ${issueText} | ${item.screenshot || '—'} |`);
    }
    md.push('', '## Usage', '');
    md.push('```powershell');
    md.push('npm run qa:hero:foldable:measured');
    md.push('npm run qa:hero:foldable');
    md.push('npm run qa:hero:foldable:portrait');
    md.push('npm run qa:hero:foldable:landscape');
    md.push('```', '');
    md.push('Optional: inject candidate CSS without editing `hero.css`:', '');
    md.push('```powershell');
    md.push('node qa/viewport-audit/run-hero-foldable-composition-qa.cjs --measured-only --override-css=qa/viewport-audit/candidates/hero-foldable-v1.css');
    md.push('```');
    fs.writeFileSync(mdOut, `${md.join('\n')}\n`);

    console.log('');
    console.log('Foldable Hero composition QA complete.');
