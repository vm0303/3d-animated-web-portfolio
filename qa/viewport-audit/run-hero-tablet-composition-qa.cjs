...
const contractPath = path.join(root, 'qa', 'viewport-audit', 'hero-tablet-composition-contract.json');
...
const outDir = earlyOutputDirValue ? path.resolve(root, earlyOutputDirValue) : path.join(root, 'qa-results', 'hero-tablet-composition');
...
if (!fs.existsSync(contractPath)) {
  console.error(`Missing tablet composition contract: ${contractPath}`);
  process.exit(1);
}
...
const tabletDeviceNames = new Set(
  (coverage.candidates || [])
    .filter((item) => String(item.deviceType || '').toLowerCase() === 'tablet')
    .map((item) => normalizeDeviceName(item.deviceName))
);

if (!tabletDeviceNames.size) {
  console.error('No tablet devices found in the current TestMU coverage set.');
  process.exit(1);
}
...
const eligibleCapture = (capture) => {
  ...
  // Tablet-phase selection comes from the current TestMU coverage inventory,
  // not from a widened phone geometry threshold. This keeps phones, foldables,
  // and posture-unverified open devices out of the tablet baseline.
  if (!tabletDeviceNames.has(normalizeDeviceName(capture.deviceName))) return false;

  const displayState = String(capture.displayState || 'standard-main-display');
  if (displayState !== 'standard-main-display') return false;
...
const main = async () => {
  ...
  try {
    ...
    console.log(`Tablet Hero composition QA: ${filteredCases.length} cases (${orientationFilter}).`);
    ...
    const md = [];
    md.push('# Tablet Hero Composition QA', '');
    ...
    console.log('');
    console.log('Tablet Hero composition QA complete.');
